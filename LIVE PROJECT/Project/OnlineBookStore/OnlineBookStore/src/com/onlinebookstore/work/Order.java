    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
            ...
            if(rs.next())
            {
                ...
                {
                    PreparedStatement pst=com.onlinebookstore.db.DB.getConnect().prepareStatement("update categories set i_quantity='"+j+"' where i_name='"+i_name+"'");
                    pst.executeUpdate();
                    PreparedStatement st=com.onlinebookstore.db.DB.getConnect().prepareStatement("insert into orderdetails(order_id,cus_id,i_pic,i_type,i_name,i_price,i_quantity,timeanddate,confirmtime,status,item_status) values(?,?,?,?,?,?,?,?,?,?,?)");
                    st.setInt(1,od);
