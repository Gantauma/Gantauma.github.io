...
try
{
    ...
    ResultSet rs=stmt.executeQuery("select * from categories where i_name='"+i_name+"'");
                ...
                {   
                    PreparedStatement ps=com.onlinebookstore.db.DB.getConnect().prepareStatement("insert into categories(cat_type,i_name,i_price,i_quantity,i_pic,admin_id) values(?,?,?,?,?,?)");
